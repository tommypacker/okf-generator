...
import { CONFIG_FILE, loadRepoOkfConfig, writeDefaultConfig } from "./config.js";
...
function resolveFlag(parsed, name, fallback) {
...
function resolveLlmOptions(parsed, repo, config) {
...
function generationModeFlag(parsed, config) {
...
function resolveGenerateWriteMode(parsed) {
...
function resolveScanOptions(parsed, config = {}) {
    return {
        packageScope: packageScopeFlag(parsed, config),
    };
}
function packageScopeFlag(parsed, config) {
...
function rejectRemovedFlags(parsed) {
...
function createProgressReporter(parsed) {
...
function findPackage(repo, target) {
    const normalizedTarget = normalizePath(target.replace(/^\.\//, "").replace(/\/+$/, "")) || ".";
    return repo.packages.find((pkg) => pkg.name === target ||
...
function printPackageExplanation(repo, pkg, scanOptions, outPath) {
...
function packageInclusionReason(repo, pkg, scanOptions) {
    if (pkg.path === ".") {
