...
export async function scanRepo(rootInput: string, options: Partial<ScanOptions> = {}): Promise<RepoInfo> {
...
async function readPackageJson(root: string, relPath: string): Promise<PackageJsonInfo | undefined> {
  const raw = await readJson<RawPackageJson>(path.join(root, relPath));
  if (!raw) {
    return undefined;
  }
...
async function detectPackages(
  ...
  for (const manifestPath of manifestFiles) {
    ...
    const manifestType = path.basename(manifestPath) as ManifestType;
...
function shouldIncludePackageManifest(
  manifestPath: string,
  rootPackageJson: PackageJsonInfo | undefined,
  options: ScanOptions,
): boolean {
...
function hasExcludedPackageSegment(packagePath: string): boolean {
...
function isExactWorkspaceMatch(packagePath: string, pattern: string): boolean {
  const normalizedPattern = normalizePath(pattern).replace(/\/+$/, "");
  return normalizedPattern === packagePath;
}

async function buildPackageScanDetails(
  root: string,
  allFiles: string[],
  pkg: PackageSummaryInput,
): Promise<PackageScanDetails> {
...
function findPackageReadme(allFiles: string[], packagePath: string): RepoFile | undefined {
...
function findPackageEntrypoints(allFiles: string[], pkg: PackageSummaryInput): RepoFile[] {
...
function buildRepoSummary(input: {
  ...
  if (input.packages.length) detected.push(`${input.packages.length} package${input.packages.length === 1 ? "" : "s"}`);
  if (input.bins.length) detected.push(`${input.bins.length} CLI entr${input.bins.length === 1 ? "y" : "ies"}`);
  if (input.docs.length) detected.push(`${input.docs.length} documentation file${input.docs.length === 1 ? "" : "s"}`);
  if (input.tests.length) detected.push(`${input.tests.length} test file${input.tests.length === 1 ? "" : "s"}`);
  if (input.ci.length) detected.push(`${input.ci.length} CI workflow${input.ci.length === 1 ? "" : "s"}`);
...
function detectDocs(files: string[]): RepoFile[] {
  return files
    .filter((file) =>
      /^README(\.|$)/i.test(file) ||
...
function detectConfigs(files: string[]): RepoFile[] {
  return files
    .filter((file) =>
...
function detectTests(files: string[]): RepoFile[] {
  return files
    .filter((file) =>
...
function detectSourceDirectories(files: string[]): RepoDirectory[] {
...
function summarizeLanguages(files: string[]): LanguageSummary[] {
...
async function readReadmeDescription(root: string, docs: RepoFile[]): Promise<string | undefined> {
...
async function readMarkdownSummary(filePath: string): Promise<string | undefined> {
...
async function latestFileTimestamp(root: string, files: string[]): Promise<string> {
  let latest = 0;
  for (const file of files) {
    try {
...
export function scriptCategory(script: ScriptInfo): string {
  const name = script.name.toLowerCase();
  if (name.includes("typecheck") || name.includes("type-check") || name === "types") return "typecheck";
...
function summarizeScriptCategories(scripts: ScriptInfo[]): string[] {
...
function isGenericReadmeSummary(value: string): boolean {
...
function packageDependencyNames(raw: RawPackageJson): string[] {
  return uniqueStrings([
...
export function titleForPackage(pkg: PackageInfo): string {
