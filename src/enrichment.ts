...
import type { LlmOptions, PackageInfo, ProgressReporter, RepoEnrichment, RepoInfo } from "./types.js";
...
async function enrichRepoQuick(
  repo: RepoInfo,
  options: LlmOptions,
  progress?: ProgressReporter,
): Promise<RepoEnrichment> {
...
async function enrichRepoExplore(
  repo: RepoInfo,
  options: LlmOptions,
  progress?: ProgressReporter,
): Promise<RepoEnrichment> {
  ...
  const packageSummaries: RepoEnrichment["packages"] = [];
  ...
  const chunkSummaries: RepoEnrichment[] = [];
...
function chunkEvidence(evidence: EvidenceFile[], chunkSize: number): EvidenceFile[][] {
...
async function collectEvidence(repo: RepoInfo, options: LlmOptions): Promise<EvidenceFile[]> {
  const allFiles = await listFiles(repo.root);
...
async function collectPackageEvidence(
  ...
  if (pkg.readme) selected.add(pkg.readme.path);
...
function isPackageConfig(file: string, prefix: string): boolean {
...
function buildPackageTreeEvidence(pkg: PackageInfo, packageFiles: string[]): string {
  const prefix = pkg.path === "." ? "" : `${pkg.path}/`;
...
function isNestedPackageFile(repo: RepoInfo, file: string): boolean {
  return repo.packages.some((pkg) => pkg.path !== "." && file.startsWith(`${pkg.path}/`));
}

function rankEvidenceFiles(repo: RepoInfo, allFiles: string[]): string[] {
  ...
  for (const file of allFiles) {
    ...
    if (IMPORTANT_FILENAMES.has(basename)) score += 100;
    if (/^README(\.|$)/i.test(file)) score += 120;
    if (file.startsWith("docs/")) score += 60;
...
function llmCachePath(userPrompt: string, options: LlmOptions): string | undefined {
...
async function readLlmCache(cachePath: string): Promise<unknown | undefined> {
  try {
...
async function writeLlmCache(cachePath: string, value: unknown): Promise<void> {
...
function buildPrompt(
  ...
  packageSummaries: RepoEnrichment["packages"] = [],
): string {
  const repoFacts = {
    name: repo.name,
...
function buildFinalRollupPrompt(
  repo: RepoInfo,
  packageSummaries: RepoEnrichment["packages"],
  chunkSummaries: RepoEnrichment[],
): string {
  return [
...
function buildPackagePrompt(repo: RepoInfo, pkg: PackageInfo, evidence: EvidenceFile[]): string {
  return [
...
function normalizeEnrichment(raw: unknown, options: LlmOptions): RepoEnrichment {
...
function assertUsableEnrichment(enrichment: RepoEnrichment): void {
  const hasOverview = Boolean(
    enrichment.overview?.summary ||
  ...
  const hasWorkflows = Boolean(
    enrichment.workflows?.development?.summary ||
...
function normalizeOverview(value: unknown): RepoEnrichment["overview"] {
  if (!isRecord(value)) {
    return undefined;
  }
  return {
...
function normalizePackage(value: unknown): RepoEnrichment["packages"][number] {
...
function normalizePackageResponse(value: unknown, fallbackPackagePath: string): RepoEnrichment["packages"][number] {
...
function mergePackageSummaries(
  explored: RepoEnrichment["packages"],
  rollup: RepoEnrichment["packages"],
): RepoEnrichment["packages"] {
  const merged = new Map<string, RepoEnrichment["packages"][number]>();
...
function normalizeSection(value: unknown): RepoEnrichment["documentation"] {
  if (!isRecord(value)) {
    return undefined;
  }
  return {
...
function collectPaths(value: unknown, output: Set<string>): void {
...
function normalizeBinTarget(packagePath: string, target: string): string {
  const prefix = packagePath === "." ? "" : `${packagePath}/`;
  return path.posix.normalize(`${prefix}${target}`).replace(/^\.\//, "");
}

function numberFromEnv(primary: string, legacy: string, fallback: number): number {
...
function llmModeFromEnv(): LlmOptions["mode"] {
  return envValue("OKFGEN_MODE", "REPO_OKF_MODE") === "explore" ? "explore" : "quick";
}

function defaultMaxFiles(mode: LlmOptions["mode"]): number {
