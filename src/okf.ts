...
function file(filePath: string, content: string): OkfFile {
...
function repositoryBody(repo: RepoInfo, enrichment?: RepoEnrichment): string {
  return joinSections(
    [
      "# Summary",
      "",
...
function architectureBody(repo: RepoInfo, enrichment?: RepoEnrichment): string {
  ...
  return joinSections(
    enrichedBullets("LLM Architecture Notes", enrichment?.overview?.architecture),
    [
      "# Source Areas",
      "",
...
function packagesIndex(repo: RepoInfo): string {
  const entries = repo.packages.map((pkg): [string, string, string] => [
    titleForPackage(pkg),
    `${packageSlug(pkg)}.md`,
...
function packageConcept(repo: RepoInfo, pkg: PackageInfo, enrichment?: RepoEnrichment): string {
  ...
  return concept({
    ...
    body: joinSections(
      ...
      enrichedBullets("Risks And Unknowns", packageEnrichment?.risksOrUnknowns),
      enrichedCitations(packageEnrichment?.citations),
      [
        "# Manifest",
        "",
        `Detected manifest: ${sourceRef(repo, pkg.manifestPath)}.`,
      ].join("\n"),
      packageDependenciesSection(repo, pkg),
      packageReadmeSection(pkg),
      packageEntrypointsSection(pkg),
      [
        "# Scripts",
        "",
        scripts,
...
function workflowConcept(
  ...
  return concept({
    ...
    timestamp: repo.scannedAt,
    body: joinSections(
      enrichedSection("LLM Summary", enrichmentSection),
      [
        "# Detected Scripts",
        "",
        scriptTable,
...
function testsBody(repo: RepoInfo): string {
  return [
    "# Test Files",
    "",
...
function releaseBody(repo: RepoInfo): string {
  const hints = [
...
function packageEntrypointsSection(pkg: PackageInfo): string {
...
function packageDependenciesSection(repo: RepoInfo, pkg: PackageInfo): string {
  ...
  const internalRows = pkg.dependencyNames
    ...
    .filter((dependency): dependency is PackageInfo => Boolean(dependency))
    .map((dependency) => [
...
function enrichmentSummary(enrichment?: RepoEnrichment): string {
  if (!enrichment) {
    return "";
  }

  const enrichedPackages = enrichment.packages.filter((pkg) =>
    pkg.summary || pkg.responsibilities.length || pkg.importantFiles.length,
  ).length;
...
function enrichedSection(title: string, section?: RepoEnrichment["documentation"]): string {
...
function enrichedCitations(citations?: string[]): string {
...
export function packageSlug(pkg: PackageInfo): string {
  return slugify(pkg.path === "." ? pkg.name : pkg.path);
}

function manifestTag(type: string): string {
...
function packageLinkByPath(repo: RepoInfo, packagePath: string): string {
...
function fileList(files: RepoFile[]): string {
  return files.map((file) => `* ${sourceRefForFile(file.path)} - ${file.kind}`).join("\n");
}

function sourceRef(repo: RepoInfo, filePath: string): string {
...
function sourceUrl(repo: RepoInfo, filePath: string): string | undefined {
  if (!repo.remoteUrl) {
    return undefined;
  }
